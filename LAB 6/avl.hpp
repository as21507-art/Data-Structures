...
class AVL
{
    ...
    private:
        int balanceFactor(Node *ptr);            // return the balance factor
        void rebalance(Node *ptr);                // rebalance a node    (if it is not already balanced)
        
        //Helper methods for rebalance. Comment out if not being used.
        void left_rotation(Node *a, Node* b);                // rotate the node a to its left
        void right_rotation(Node *a, Node* b);            // rotate the node a to its right
    
        void left_right(Node* a, Node* b, Node* c);
        void right_left(Node* a, Node* b, Node* c);
    
    
        void copyKey(Node* a, Node* b);
    
    public:
        //Required for the Autograder. DO NOT remove or implement it
        void preorder(Node *ptr,ostream &sstr);
};
//===============================================================
//Print a 2D Tree
void AVL::display()
...
//=====================================================================
//Define required methods of AVL class below this line
...
void AVL::copyKey(Node* ptr1, Node* ptr2){
...
void AVL::remove(Node* ptr, int key){
    ...
    // If the value did not exist in the AVL, do nothing
    ...
    // Store the node to rebalance from
    ...
    rebalance_node = old_node->parent;
    ...
    // Rebalance the tree from the parent of deleted node
...
// helper Method right_rotation: Rotates node a to the right with node b as its left child
// Input: Node a (parent) and Node b (left child to become new parent)
void AVL::right_rotation(Node* a, Node* b){
    
    Node* cp = a->parent;
    
    // Changing the new root to be b
    if(cp != nullptr) {
        if(cp->key > a->key) cp->left = b;
        else cp->right = b;
    } else {
        // a is the root node, update root
        this->root = b;
    }
    
    b->parent = cp;
 
    // Changing the right subtree of b to have a as its parent
    a->left = b->right;
    
    // FIX: Check if b->right is null before accessing its parent pointer
    if(b->right != nullptr)
        b->right->parent = a;

    // Changing a to be the child of b
    a->parent = b;
    b->right = a;
...
// helper Method left_rotation: Rotates node a to the left with node b as its right child
// Input: Node a (parent) and Node b (right child to become new parent)
void AVL::left_rotation(Node* a, Node* b){
    ...
    // Changing the left subtree of b to have a as its parent
    a->right = b->left;
    
    // FIX: Check if b->left is null before accessing its parent pointer
    if(b->left != nullptr)
        b->left->parent = a;
...
// helper Method left_right: Performs a left rotation followed by right rotation
// Input: Node a (unbalanced), Node b (left child), Node c (right child of b)
void AVL::left_right(Node* a, Node* b, Node* c){
    
    // Perform left rotation on the left subtree
    left_rotation(b, c);
    
    // Then perform right rotation on the unbalanced node
    right_rotation(a, a->left);
}


// helper Method right_left: Performs a right rotation followed by left rotation
// Input: Node a (unbalanced), Node b (right child), Node c (left child of b)
void AVL::right_left(Node* a, Node* b, Node* c){
    
    // Perform right rotation on the right subtree
    right_rotation(b, c);
    
    // Then perform left rotation on the unbalanced node
    left_rotation(a, a->right);
}


// balanceFactor Method: Calculates the height difference between left and right subtrees
// Input: A node pointer
// Output: Balance factor (height of left subtree - height of right subtree)
int AVL::balanceFactor(Node* ptr){
    return height(ptr->left) - height(ptr->right);
}

// rebalance Method: Uses rotations to balance a tree where the magnitude of balance factor is greater than 1
// Input: A node pointer to start rebalancing from
void AVL::rebalance(Node* ptr){
    ...
    // LEFT HEAVY: Balance factor > 1 indicates left subtree is too heavy
    if (bf > 1){
        
        // Check the balance factor of the left child to determine rotation type
        int left_bf = balanceFactor(ptr->left);
        
        // Left-Left case: Single rotation to the right
        if (left_bf >= 0){
            right_rotation(ptr, ptr->left);
        }
        
        // Left-Right case: First rotate left child to the left, then rotate current node to the right
        else{
            left_right(ptr, ptr->left, ptr->left->right);
        }
        
    }
    
    // RIGHT HEAVY: Balance factor < -1 indicates right subtree is too heavy
    // FIX: Changed from (bf < 2) to (bf < -1)
    if (bf < -1){
        
        // Check the balance factor of the right child to determine rotation type
        int right_bf = balanceFactor(ptr->right);
        
        // Right-Right case: Single rotation to the left
        if (right_bf <= 0){
            left_rotation(ptr, ptr->right);
        }
        
        // Right-Left case: First rotate right child to the right, then rotate current node to the left
        else{
            right_left(ptr, ptr->right, ptr->right->left);
        }
    }
    
    // Continue rebalancing up the tree by rebalancing the parent
